...
build:
	docker-compose down && docker-compose build mongo_db dg_server debriefing_app control_server navigation_app

# Start DroneGuard Project [see specific ports in docker-compose file]
start:
	docker-compose up mongo_db dg_server debriefing_app control_server navigation_app && open http://localhost:3000 http://localhost:3003
